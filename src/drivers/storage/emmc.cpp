...
bool intel_enable_power(Controller& ctrl) {
    ...
    *reinterpret_cast<volatile uint32_t*>(regs + 0xA0) = 0x1;
    udelay();
    *reinterpret_cast<volatile uint32_t*>(regs + 0xA4) = 0x1;
    udelay();
    return true;
}
...
bool set_clock(Controller& ctrl, uint32_t hz) {
    ...
    uint32_t base = ctrl.base_clock_hz;
    if (base == 0) {
        base = 50000000;
    }

    uint16_t divisor = build_divisor(base, hz);
    uint16_t clock = divisor | SDHCI_CLOCK_INT_EN;
    write16(ctrl, SDHCI_CLOCK_CONTROL, clock);

    if (!wait_for_bits(ctrl, SDHCI_CLOCK_CONTROL, SDHCI_CLOCK_INT_STABLE, 10000)) {
        log_message(LogLevel::Warn,
                    "eMMC: controller %02x:%02x.%01x clock did not stabilize",
...
void enable_power(const Controller& ctrl) {
    ...
    udelay();
    write8(ctrl, SDHCI_POWER_CONTROL, 0x0F);
    udelay();
}
...
bool wait_cmd_ready(const Controller& ctrl) {
    return wait_for_clear(ctrl, SDHCI_PRESENT_STATE,
                          SDHCI_PRESENT_INHIBIT_CMD, 10000);
}

bool wait_data_ready(const Controller& ctrl) {
    return wait_for_clear(ctrl, SDHCI_PRESENT_STATE,
                          SDHCI_PRESENT_INHIBIT_DATA, 10000);
}
...
bool send_command(const Controller& ctrl,
    ...
    uint16_t cmd = response_flags(response) | SDHCI_CMD_TYPE_NORMAL |
    ...
    if (!wait_for_interrupt(ctrl, SDHCI_INT_CMD_COMPLETE, 100000)) {
        log_message(LogLevel::Warn,
                    "eMMC: command %u timed out", static_cast<unsigned>(index));
        return false;
    }
...
bool init_card(Controller& ctrl, Device& device) {
    if (!reset_controller(ctrl)) {
        log_message(LogLevel::Error,
                    "eMMC: failed to reset controller %02x:%02x.%01x",
                    ctrl.pci.bus, ctrl.pci.slot, ctrl.pci.function);
        return false;
    }
    ...
    if (!wait_cmd_ready(ctrl)) {
        log_message(LogLevel::Warn,
                    "eMMC: controller %02x:%02x.%01x command line stuck busy",
                    ctrl.pci.bus, ctrl.pci.slot, ctrl.pci.function);
        return false;
    }
    if (!wait_data_ready(ctrl)) {
        log_message(LogLevel::Warn,
                    "eMMC: controller %02x:%02x.%01x data line stuck busy",
                    ctrl.pci.bus, ctrl.pci.slot, ctrl.pci.function);
        return false;
    }
