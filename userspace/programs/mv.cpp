...
int main(uint64_t arg_ptr, uint64_t) {
    ...
    if (!neutrino_parse_two_args(args, source, sizeof(source), dest, sizeof(dest))) {
        neutrino_write_line(console, "usage: mv <source> <dest>");
        return 1;
    }

    if (!neutrino_copy_file(source, dest)) {
        neutrino_write(console, "mv: unable to copy ");
        neutrino_write(console, source);
        neutrino_write(console, " to ");
        neutrino_write_line(console, dest);
        return 1;
    }
    if (file_remove(source) < 0) {
        file_remove(dest);
        neutrino_write(console, "mv: unable to remove ");
        neutrino_write_line(console, source);
        return 1;
    }
    return 0;
}
